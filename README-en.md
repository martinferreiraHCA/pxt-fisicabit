...
| Send without time | `send to fisicabit.com without time [value] every [100] ms` | Sends only the measured values (2, 3 and 4-value variants). On fisicabit.com disable "Micro:bit sends timestamp": the page uses the browser clock |
...
| Send without time | `send to fisicabit.com via Bluetooth without time [value] every [100] ms` | Sends only the measured values (2, 3 and 4-value variants). On fisicabit.com disable "Micro:bit sends timestamp" |
